...
public class GetBorrowedUserBookLoansResource {
    ...
    public Response getBorrowedUserBookLoans(@HeaderParam("Database-Type") String databaseType, @Context ContainerRequestContext requestContext) {
        ...
        if (databaseType.equalsIgnoreCase(DatabaseTypeEnum.SQL.toString())) {
            ...
            return getBorrowedUserBookLoansMongoDB(userId);
        }
    }

    public Response getBorrowedUserBookLoansMongoDB(String userId) {
        ...
        List<LoanedBook> loanedBooks = loanedBookService.findCurrentBorrowedBooksForUser(user);
