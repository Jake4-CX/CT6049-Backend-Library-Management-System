...
@Path("/users/me/loans/history")
public class GetHistoryUserBookLoansResource {

    @GET
    @RestrictedRoles({User.Role.USER, User.Role.ADMIN})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getHistoryUserBookLoans(@HeaderParam("Database-Type") String databaseType, @Context ContainerRequestContext requestContext) {
        ...
        if (databaseType.equalsIgnoreCase(DatabaseTypeEnum.SQL.toString())) {
            return getHistoryUserBookLoansSQL(userId);
        } else {
            return getHistoryUserBookLoansMongoDB(userId);
        }
    }

    public Response getHistoryUserBookLoansMongoDB(String userId) {
        ...
        List<LoanedBook> loanedBooks = loanedBookService.findHistoricLoanedBooksForUser(user);
    ...
    public Response getHistoryUserBookLoansSQL(String userId) {
        Map<String, Object> response = new HashMap<>();
        ...
        HashMap<String, Object>[] loanedBooks = loanedBookService.findHistoricLoanedBooksForUser(Integer.valueOf(userId));
